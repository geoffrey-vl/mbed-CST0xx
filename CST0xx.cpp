...
CST0xx::CST0xx(mbed::I2C* i2c, PinName interruptPin, PinName resetPin) : 
    CST0xx(i2c, interruptPin, resetPin, nullptr)
{
}

CST0xx::CST0xx(mbed::I2C* i2c, PinName interruptPin, PinName resetPin, mbed::Callback<void(struct ts_event)> touch_event_callback) : 
    _i2c(i2c),
...
void CST0xx::setCallBack(mbed::Callback<void(struct ts_event)> touch_event_callback)
{
    _touch_event_callback = touch_event_callback;
}

void CST0xx::enableIRQ()
{
    // Request the shared queue
    EventQueue *sharedEventQueue = mbed::mbed_event_queue();
    // attach touch handler to interrupt pin
    _touchInterrupt.fall(sharedEventQueue->event(this, &CST0xx::onTouchInterrupt)); 
}

void CST0xx::disableIRQ()
{
    // detach touch handler to interrupt pin
    _touchInterrupt.fall(nullptr); 
}

void CST0xx::onTouchInterrupt()
{
    //When IRQ pin was rised the touchpad device has wakened up.
    ...
    switch (data.type)
    {
		case CST0xx::EVENT_TYPE_TOUCH:
			tr_info("TOUCH X: %u Y: %u", event.x, event.y);
			break;
		case CST0xx::EVENT_TYPE_LONG_TOUCH:
			tr_info("TOUCH LONG X: %u Y: %u", event.x, event.y);
			break;
		case CST0xx::EVENT_TYPE_SWIPE_Y_POSITIVE:
			tr_info("SWIPE y+ X: %u Y: %u", event.x, event.y);
			break;
		case CST0xx::EVENT_TYPE_SWIPE_X_POSITIVE:
			tr_info("SWIPE x+ X: %u Y: %u", event.x, event.y);
			break;
		case CST0xx::EVENT_TYPE_SWIPE_Y_NEGATIVE:
			tr_info("SWIPE y- X: %u Y: %u", event.x, event.y);
			break;
		case CST0xx::EVENT_TYPE_SWIPE_X_NEGATIVE:
			tr_info("SWIPE x- X: %u Y: %u", event.x, event.y);
			break;
    }
	*/
...
uint16_t CST0xx::i2c_reg_write(uint8_t i2c_addr, uint8_t reg_addr, uint8_t *reg_data,
                                        uint16_t length)
{
...
uint16_t CST0xx::i2c_reg_read(uint8_t i2c_addr, uint8_t reg_addr, uint8_t *reg_data,
                                       uint16_t length)
{
